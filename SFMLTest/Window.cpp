...
Window::Window() 
	: windowTitle("Default Title"), 
	  windowSize(640, 480), 
	  isDone(false), 
	  isFullscreen(false)// : <FILL THIS INITIALIZATION LIST with default values for isDone and isFullScreen>
{ 
...
void Window::create()
{
	...
	// 创建一个sf:：VideoMode对象，将两个参数传递给其构造函数：windowsSize.x和windowsSize.y。
	sf::VideoMode videoMode{ windowSize.x, windowSize.y };
    //   -> bonus: do this using "direct brace initialization" 使用“直接括号初始化”来完成此操作
	...
	//在sf:：RenderWindow窗口中调用函数create（），按以下顺序传递参数：视频模式、窗口标题和样式。
